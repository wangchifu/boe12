                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="headingTwo">
                          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                            <i class="fas fa-plus text-info"></i> 新增選單(按一下)
                          </button>
                        </h2>
                        <div id="collapseTwo" class="accordion-collapse collapse" aria-labelledby="headingTwo" data-bs-parent="#accordionExample">
                          <div class="accordion-body">
                            <form action="{{ route('menu_add') }}" method="POST" id="menu_form">
                                ...
                                <a class="btn btn-primary btn-sm" onclick="if(confirm('確定儲存嗎？登出才會生效喔！')) document.getElementById('menu_form').submit();">
                            ...
                            @foreach($menus as $menu)
                                <tr>
                                    ...
                                    @if($menu->type==1)
                                        ...
                                        @endphp
                                        <td><i class="fas fa-folder text-warning"></i> <a href="{{ route('menu',['id'=>$menu->id]) }}">{{ $menu->name }}</a></td>
                                    @endif
                                    @if($menu->type==2)
                                        <td><i class="fas fa-link text-primary"></i> {{ $menu->name }}</td>
                                    @endif
                                    <td>
                                        @if($menu->type==1)
                                            {{ $sub_menu_number }}
                                    ...
                                    <td>                    
                                        ...
                                        <a href="{{ route('menu_del',$menu->id) }}" class="btn btn-danger btn-sm" onclick="confirm('連同底下的一起刪除喔！登出才會生效喔！')">刪除</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
