                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table rwd-table" style="word-break: break-all;">
                                ...
                                <tr>
                                    <th colspan="6" class="text-right" style="text-align: right;"><button onclick="if(confirm('您確定打勾的都要簽收嗎?')) more_post();else return false">打勾者批次簽收</button></th>
                                </tr>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td data-th="主旨">
                                            ...
                                            @if( $post_school->post->situation ===4 )
                                                <span style="color:red">[公告作廢]</span> <strike class="text-primary"><a href="javascript:open_post('{{ route('posts.show',$post_school->post_id) }}','新視窗')">{{ str_limit($post_school->post->title,160) }}</a></strike>
                                            @else
                                                <a href="javascript:open_post('{{ route('posts.show',[$post_school->post_id,$post_school->id]) }}','新視窗')">
                                                    <span style="color:#000088">
                                                        {{ str_limit($post_school->post->title,160) }}
                                                    </span>
                                                </a>
                                            @endif
                                        </td>
                                        ...
                                        <form action="{{ route('posts.signed', ['ps_id' => $post_school->id]) }}" method="POST" id="sign_check_form{{ $post_school->post_id }}">
                                            ...
                                            <button class="btn btn-success btn-sm" type="button"  onclick="if(confirm('您確定簽收嗎?')) signcheck{{ $post_school->post_id }}();else return false">
                                                簽收
                                            </button>
                                            ...
                                            <script>
                                                function signcheck{{ $post_school->post_id }}() {
                                                    var user_power = $("#h_user_power").val();
                                                    if (user_power == "B") {
                                                        $("#sign_check_form{{ $post_school->post_id }}").submit();
                                                    } else {
                                                        alert('您沒有簽收的權限');
                                                    }
                                                }
                                ...
                                <tr>
                                    <th colspan="6" class="text-right"  style="text-align: right; style="text-align: right;><button onclick="if(confirm('您確定打勾的都要簽收嗎?')) more_post();else return false">打勾者批次簽收</button></th>
                                </tr>
                            </table>
                        </div>                        
